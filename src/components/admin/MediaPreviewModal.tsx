...
function previewUrl(media: MediaAssetModel) {
  if (media.watermarkedPath) return resolvePublicFileUrl(media.watermarkedPath);
  if (media.thumbnailPath) return resolvePublicFileUrl(media.thumbnailPath);
  return "";
}
      ...
      >
        <div className="flex items-center justify-between border-b border-white/10 px-5 py-4">
          <div className="min-w-0">
            ...
            <p className="mt-1 font-mono text-xs text-white/45">{media.id}</p>
          </div>
        ...
        <div className="bg-[radial-gradient(900px_circle_at_20%_0%,rgba(255,70,85,0.16),transparent_55%),radial-gradient(900px_circle_at_80%_20%,rgba(255,255,255,0.08),transparent_55%)] p-5">
          {url ? (
            isVideo ? (
              <video controls src={url} className="max-h-[72vh] w-full rounded-2xl border border-white/10 bg-black/40 object-contain" />
            ) : (
              <img src={url} alt="" className="max-h-[72vh] w-full rounded-2xl border border-white/10 bg-black/40 object-contain" />
            )
          ) : (
            <div className="grid min-h-[40vh] place-items-center rounded-2xl border border-white/10 bg-white/[0.04]">
              <div className="text-center">
                ...
                <p className="mt-1 text-xs text-white/55">Esta mídia ainda não possui um arquivo para pré-visualização.</p>
              </div>
            </div>
          )}
        </div>
